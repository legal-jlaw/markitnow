...
function PurchasePanel({ mark, trademarks, loading, isMobile }) {
  ...
  const [showDisclaimer, setShowDisclaimer] = useState(false); // clickwrap modal
  const [disclaimerChecked, setDisclaimerChecked] = useState(false);
  const [disclaimerAccepted, setDisclaimerAccepted] = useState(devMode); // skip in dev
  const [pendingDownload, setPendingDownload] = useState(null); // { fn, args }

  function triggerDownload(fn, ...args) {
    if (disclaimerAccepted) { fn(...args); return; }
    setPendingDownload({ fn, args });
  ...
  function confirmDisclaimer() {
    ...
    if (pendingDownload) { pendingDownload.fn(...pendingDownload.args); setPendingDownload(null); }
  }
          ...
          {!isPaid ? (
            ...
            <>
              ...
              <div style={{ background: "#f0fff4", borderRadius: 10, padding: 14, border: "1px solid #9ae6b4", display: "flex", gap: 10, alignItems: "center", marginBottom: 10 }}>
                ...
                <button onClick={() => triggerDownload(generateReportPDF, mark, report, report._agentData)} style={{ padding: "9px 16px", background: "#2d7a4f", color: "#fff", border: "none", borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>Download PDF</button>
              </div>
            </>
          )}
          ...
          <div style={{ background: "#fffdf7", borderRadius: 10, padding: 14, border: "2px solid #c9a84c" }}>
            {!isPaid ? (
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                ...
                <button onClick={() => triggerDownload(generateMemoPDF, mark, memo)} style={{ padding: "9px 16px", background: "#c9a84c", color: "#0a0a0a", border: "none", borderRadius: 8, fontWeight: 800, fontSize: 12, cursor: "pointer" }}>Download PDF</button>
              </div>
            )}
          </div>
        </div>
  ...
  if (showDisclaimer) {
    return (
      <>
        ...
        {/* Modal */}
        <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 2001, background: "#fff", borderRadius: 14, padding: 28, width: 420, maxWidth: "90vw", boxShadow: "0 24px 80px rgba(0,0,0,0.35)", fontFamily: "Poppins, sans-serif" }}>
          {/* Gold accent bar */}
          <div style={{ height: 4, background: "#c9a84c", borderRadius: "4px 4px 0 0", margin: "-28px -28px 20px -28px" }} />
          <div style={{ fontSize: 18, fontWeight: 800, color: "#111", marginBottom: 6 }}>Before You Download</div>
          <div style={{ fontSize: 12, color: "#666", lineHeight: 1.7, marginBottom: 16 }}>
            Please read and acknowledge the following before downloading this document:
          </div>
          <div style={{ background: "#f8faf9", borderRadius: 8, padding: "12px 14px", marginBottom: 18, fontSize: 11, color: "#444", lineHeight: 1.8 }}>
            <div style={{ marginBottom: 6 }}>1. This document is <strong>generated by artificial intelligence</strong> and is provided for <strong>informational purposes only</strong>.</div>
            <div style={{ marginBottom: 6 }}>2. It does <strong>not constitute legal advice</strong> and does not create an attorney-client relationship.</div>
            <div>3. Results are based on USPTO data at the time of search and <strong>may not reflect all existing conflicts</strong>. You should consult a licensed trademark attorney before making any filing decisions.</div>
          </div>
          {/* Checkbox */}
          <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", marginBottom: 18 }}>
            <input
              ...
              style={{ marginTop: 2, width: 16, height: 16, accentColor: "#c9a84c", flexShrink: 0, cursor: "pointer" }}
            />
            <span style={{ fontSize: 12, color: "#333", lineHeight: 1.6 }}>
              I understand this document is AI-generated and does not constitute legal advice. I agree to these terms.
            </span>
          </label>
          {/* Upsell line */}
          <div style={{ fontSize: 11, color: "#999", marginBottom: 16, textAlign: "center" }}>
            Want an attorney to review this?{" "}
            <a href={`/file?mark=${encodeURIComponent(mark)}`} style={{ color: "#c9a84c", fontWeight: 700, textDecoration: "none" }}>
              File your application for $399 →
            </a>
          </div>
          {/* Buttons */}
          <div style={{ display: "flex", gap: 10 }}>
            <button
              ...
              style={{ flex: 1, padding: "10px", background: "#f4f4f4", border: "none", borderRadius: 8, fontWeight: 600, fontSize: 12, color: "#666", cursor: "pointer" }}
            >
              Cancel
            </button>
            <button
              ...
              style={{ flex: 2, padding: "10px", background: disclaimerChecked ? "#111" : "#ccc", border: "none", borderRadius: 8, fontWeight: 800, fontSize: 12, color: "#fff", cursor: disclaimerChecked ? "pointer" : "not-allowed", transition: "background 0.2s" }}
            >
