...
function rapidHeaders(apiKey) {
  return { "x-rapidapi-key": apiKey, "x-rapidapi-host": RAPIDAPI_HOST };
}
...
export async function stepRetrieve(mark, classCode, apiKey) {
  const firstWord = mark.trim().split(/\s+/)[0];
  const phonetic = phoneticVariant(mark);

  const queries = [
    queryUSPTO(mark, "all", apiKey),
    queryUSPTO(mark, "active", apiKey),
    queryUSPTO(firstWord, "active", apiKey),
  ];
...
export async function runAnalysis(mark, goodsServices, classCode, useType) {
  ...
  if (!rapidKey) throw new Error("RAPIDAPI_KEY not configured");
